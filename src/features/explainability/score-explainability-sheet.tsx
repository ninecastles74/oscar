...
export function ScoreExplainabilitySheet({
  open,
  onOpenChange,
  explainability,
}: {
  ...
  const entityTitle =
    explainability.entityType === "article"
      ? "Article reliability"
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader>
          <SheetTitle className="font-serif text-2xl">{entityTitle}</SheetTitle>
          <SheetDescription className="text-left">
            {explainability.entityLabel} — score {explainability.overallScore}/100
          </SheetDescription>
        </SheetHeader>

        <div className="mt-6 space-y-8 pb-10">
          <section>
            <p className="text-xs leading-relaxed text-muted-foreground">{explainability.disclaimer}</p>
          </section>
          ...
          <section>
            ...
            <div className="mt-4 space-y-3">
              {explainability.calculationSteps.map((step) => (
                <div key={step.categoryId} className="rounded-md border bg-card p-3">
          ...
          <section>
            <h3 className="font-serif text-lg font-semibold">Confidence explanation</h3>
            <p className="mt-2 text-sm leading-relaxed text-foreground/85">
            ...
            <section>
              ...
              <div className="mt-3 space-y-2">
                {explainability.corroboratingSources.map((s) => (
                  <div key={s.sourceId} className="rounded-md border bg-card p-3 text-sm">
                    ...
                    {s.excerpts.slice(0, 2).map((ex, i) => (
                      <p key={i} className="mt-2 text-xs italic text-foreground/75">
                        "{ex}"
                      </p>
                    ))}
                  </div>
            ...
            <section>
              ...
              <div className="mt-3 space-y-2">
                {explainability.contradictionHistory.map((c, i) => (
                  <div key={i} className="rounded-md border border-destructive/20 bg-destructive/5 p-3 text-sm">
                    <div className="text-xs font-semibold uppercase text-destructive">
                      {c.severity} · {c.sourceNames.join(" vs ")}
                    </div>
                    <p className="mt-1 font-medium">{c.claimText.slice(0, 160)}</p>
              ...
              <div className="mt-3 space-y-2">
                {explainability.omittedContext.map((o, i) => (
                  <div key={i} className="rounded-md border border-warning/30 bg-warning/5 p-3 text-sm">
                    <p className="font-medium">{o.claimText.slice(0, 160)}</p>
          ...
          {explainability.historicalChanges.length > 0 && (
            <section>
              <h3 className="flex items-center gap-2 font-serif text-lg font-semibold">
                <History className="h-4 w-4" /> Historical scoring changes
              </h3>
              <div className="mt-3 space-y-2">
                {explainability.historicalChanges.map((h) => (
                  <div key={h.version} className="flex gap-3 rounded-md border bg-card p-3 text-sm">
    ...
    <section>
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-3 max-h-64 space-y-2 overflow-y-auto">
          {items.slice(0, 12).map((e) => (
            <div
              ...
              className={cn(
                ...
                tone === "support" ? "border-success/20 bg-success/5" : "border-destructive/20 bg-destructive/5",
              )}
            >
              ...
              <p className="mt-1 text-xs leading-relaxed">{e.claimText.slice(0, 120)}</p>
              <p className="mt-2 text-xs italic text-foreground/80">"{e.excerpt.slice(0, 200)}"</p>
              {e.url && (
                <a
                  href={e.url}
                  target="_blank"
          ...
          {items.length > 12 && (
            <p className="text-xs text-muted-foreground">+{items.length - 12} more citations</p>
          )}
        </div>
      )}
    </section>
